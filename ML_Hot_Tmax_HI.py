...
"""
Zero out temps less than a 'hot' threshold
"""
hot_threshold=80
inputs=xr.where(inputs<hot_threshold,hot_threshold-5,inputs)
target=xr.where(target<hot_threshold,hot_threshold-5,target)
...
scaler_input= MinMaxScaler(feature_range=(0,1));scaler_target= MinMaxScaler(feature_range=(0,1))
...
#t_board = [keras.callbacks.TensorBoard(log_dir='/cpc/home/gjennrich/my_tf_dir/'+var+'_hot_member_2layer',histogram_freq=1,embeddings_freq=1,)]
...
Model.save(project_dir+'Scripts/ML_heat/'+var+'_Hot_Model')# Save model for future use
...
#Score rmse
rmse=[];rmse_GEFS=[];nn_map_list=[];input_hot_list=[];target_nn_hot_list=[];target_gefs_hot_list=[]
for d in tqdm(range(nn_map_1d.shape[0])):
	"""
	mask out anything less than the 'hot threshold'
	"""
	nn_map_=nn_map_1d[d,:][np.where(nn_map_1d[d,:]>hot_threshold)[0]]#NN forecast for hot points
	input_hot=input_test_.values[d,:][np.where(input_test_.values[d,:]>hot_threshold)[0]] #GEFS forecast for hot points
	target_nn_hot=target_test_.isel(time=d).values[np.where(nn_map_1d[d,:]>hot_threshold)[0]] #CDAS Obs for NN hot points
	target_gefs_hot=target_test_.isel(time=d).values[np.where(input_test_.values[d,:]>hot_threshold)[0]] #CDAS Obs for GEFS hot points
	#nn_map_list.append(nn_map_);input_hot_list.append(input_hot);target_nn_hot_list.append(target_nn_hot);target_gefs_hot_list.append(target_gefs_hot)
	#score in
	rmse_day= sqrt(mean_squared_error(nn_map_,target_nn_hot))
	rmse.append(rmse_day)
	rmse_day= sqrt(mean_squared_error(input_hot,target_gefs_hot))
...
print('Correlation: '+str(np.corrcoef(rmse,rmse_GEFS)[0,1]))
#RMSE Maps

gefs_maps=RMSE(xr.where(input_2d<hot_threshold,np.nan,input_2d).values,target_2d.values,axis=0)
NN_maps=RMSE(xr.where(nn_map<hot_threshold,np.nan,nn_map).values,target_2d.values,axis=0)
...
fig, axarr = plt.subplots(3, 1, figsize=(10, 8), subplot_kw={'projection': crs}) #make the map bigger than the table
#hemispheric view
ax1=axarr[0];ax2=axarr[1];ax3=axarr[2];

plot_background(ax1);plot_background(ax2);plot_background(ax3)
pick_date=-23#date(2011,7,29)
...
cf1 = ax1.contourf(input_plot.longitude, input_plot.latitude, input_plot.isel(time=pick_date).transpose(), 17,cmap=cmap,levels=np.linspace(hot_threshold,120,17),vmax=120, vmin=hot_threshold,extend='both')
ax2.set_title('NN Corrected '+var+' Forecast RMSE: '+str(round(rmse[pick_date],2)))
cf2 = ax2.contourf(nn_map.longitude,nn_map.latitude,  nn_map.isel(time=pick_date).transpose(),17,cmap=cmap,levels=np.linspace(hot_threshold,120,17),vmax=120, vmin=hot_threshold,extend='upper')
ax3.set_title('Observation: '+pd.DatetimeIndex(target_2d.time.values)[pick_date].strftime('%b %d %Y'))
cf3 = ax3.contourf(target.longitude, target.latitude,  target_plot.isel(time=pick_date).transpose(), 17,cmap=cmap,levels=np.linspace(hot_threshold,120,17),vmax=120, vmin=hot_threshold,extend='both')
...
plt.savefig(var+'_hot_NN_member_ex.png')
plt.show()
...
plt.savefig(var+'_rmse_map_hot_members.png')
plt.show()
