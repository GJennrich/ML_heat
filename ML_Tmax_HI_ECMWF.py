...
Using ECMWF Tmax to give a more skillful Tmax/HI forecast
...
def Get_dateime_date_EC(var_time):#time coord
	dates_2021=[];time_coords=[]
	start_date=date(2021,5,13);end_date=date(2021,8,16)
	date_range=pd.date_range(start_date,end_date)
	for t in date_range:
		if t.weekday() in [0,3]:
			dates_2021.append(t)
	for year in range(2001,2021): #2001-2020 dates
		for t in dates_2021:
			time_coords.append(date(year,t.month,t.day))
	var_time['time']=(np.asarray(time_coords)).astype('datetime64')
	return var_time['time']

def Get_forecast_dates(forecast_time_xr,dayofarray=-1):#for a given initalization date, get the forecast dates
...
#
"""
Read in Evan's ECMWF data and his obs data For Tmax
"""
var='Tmax'#Tmax or HI
if var=='Tmax':
	fcst1='/cpc/gth/GTH_DATABASE/WEEK2_HEAT/ECMWF_SEHOS/ECMWF_CY47r2_rfcst.nc'
	obs1='/cpc/home/evan.oswald/R1_SEHOS/CDAS_1x1_reanalysis_airT.nc'
	forecast=Ds(fcst1)['max_air_temp_2m']#;intal_date=Ds(fcst1)['issue_date']
	...
	forecast=Ds(fcst1)['max_heat_index_2m']#;intal_date=Ds(fcst1)['issue_date']
	obs=Ds(obs1)['max_heat_index_2m'];obs_date=Ds(obs1)['issue_date']

test=Ds('/cpc/model_reforecasts/ecens-ext/cycle-47r2/netcdf/ecens-ext_tmax_0513_00z.nc')
#Get datetime objects for time
forecast['time']=Get_dateime_date_EC(forecast.time)
obs['time']=Get_dateime_date(obs_date)
#Get dates for the forecast
forecast['forecast_day']=np.arange(8,15);forecast['member']=np.arange(1,12)#give the day+ forecast number
forecast['longitude']=np.arange(231,302);obs=obs[:,::-1,:]#get the same lon and lats!
forecast=forecast.rename({'forecast_day':'fcst_date','member':'ensemble'})
forecast=forecast.interp(latitude=np.arange(20.0,50.1,1.0))
#test
Get_forecast_dates(forecast['time'],0)

"""
Cut down to 2000-2020, to remove 'bad' ECMWF data
Remove missing Obseravtions (June and July 2015)
"""
forecast=forecast.sel(time=slice(date(2000,1,1),date(2020,12,31)))
#obs=obs.sel(time=forecast.time)
...
41811= 543 int dates * 7 forecast dates * 11 ens members
"""
ens_mean=False;day_specific=False
if ens_mean:
	t_max_ens_mean=forecast.mean(dim='ensemble')
	inputs=t_max_ens_mean.stack(fcst=('time','fcst_date'))
elif day_specific:
	inputs=forecast.sel(fcst_date=11).stack(fcst=('time','ensemble'))
	#inputs=forecast.stack(fcst=('time','ensemble'))
else:
	inputs=forecast.stack(fcst=('time','fcst_date','ensemble'))
input_dates=[]
for d in tqdm(range(inputs.shape[-1])):
	date_add=pd.DatetimeIndex(inputs.time.values)[d]+timedelta(int(inputs.fcst_date.values[d]))
	#date_add=pd.DatetimeIndex(inputs.time.values)[d]+timedelta(8)
...
Input: ECMWF Tmax Ens mean forecasts
Target: Observed Tmax
Samples:  2000-2020 Daily ECMWF forecasts Day +8 to +14 (each as indivdual sample)
...
epochs = 20#HI-15, Tmax-20
activation_function =tf.nn.tanh
from tensorflow.keras.models import Model
input1 = Input(shape=(1306,)) #ECMWF temp
...
#t_board = [keras.callbacks.TensorBoard(log_dir='/cpc/home/gjennrich/my_tf_dir/'+var+'_ens_member_2layer_ECMWF2',histogram_freq=1,embeddings_freq=1,)]
...
#Model.save(project_dir+'Scripts/ML_heat/'+var+'_ALL_Model_ECMWF')#save the model for future use
...
for d in range(nn_map_1d.shape[0]):
	...
	rmse_ECMWF.append(rmse_day)
print('Mean ML RMSE: ' +str(np.asarray(rmse).mean()))
print('Mean ECMWF RMSE: ' +str(np.asarray(rmse_ECMWF).mean()))

#RMSE Maps
ECMWF_maps=RMSE(input_2d.values,target_2d.values,axis=0)
...
pick_date=-20#date(2011,7,29)
#print_skill=[hss_NN[pick_date],hss_model_ens[pick_date],hss_LR[pick_date]]
ax1.set_title('ECMWF '+var+' Forecast RMSE: '+str(round(rmse_ECMWF[pick_date],2)))
...
ax1.set_title('ECMWF '+var+' Forecast RMSE: '+str(round(np.nanmean(rmse_ECMWF),2)),fontsize='18')
cf1 = ax1.pcolormesh(land_mask_fix.longitude, land_mask_fix.latitude, ECMWF_maps.transpose(),cmap=cmap,vmax=18, vmin=2)
...
plt.savefig(var+'_rmse_map_ens_members_ECMWF.png')
plt.show()
...
plt.scatter((nn_map.time), rmse_ECMWF, color='green',label='ECMWF RMSE ('+"{:.1f}".format(np.nanmean(rmse_ECMWF))+')')
